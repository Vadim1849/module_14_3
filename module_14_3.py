...
api = "8078416681:AAFxHDsEKaHcinE8Uq9aPxjsSPOB9Tj3WDM"
bot = Bot(token=api)
...
async def get_buying_list(message: types.Message):
    ...
    for product in products:
        ...
        await message.answer_photo(photo=InputFile(product["image_path"]), caption=long_string)

    inline_kb = InlineKeyboardMarkup()
    inline_kb.row(*(InlineKeyboardButton(product["name"], callback_data='product_buying') for product in products))
